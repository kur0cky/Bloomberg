...
factor.tmp <- read_csv("E:/Git/Bloomberg/data/factors_8.csv") %>% 
  select(date,CMA, HML, JPY_USD, MOM, RMW, SMB, TOPIX)
```
...
```{r grid search}
gs <- tibble(rho = c(seq(0,0.2,0.025),seq(0.3,1,0.1),seq(2,10,1),20)) %>% 
...
gs %>% 
  ...
  labs(y="SSE")+
  geom_hline(yintercept = min(gs$sse), size = 1, colour="red", linetype="dashed")+
  annotate("text",label="SSE=434.5",x=9,y=min(gs$sse)+4)+
  geom_vline(xintercept = 0.125, size=1, colour="blue", linetype="dashed")+
  annotate("text", label="rho=0.1", x=2, y=490)


```

```{r plot}
factor.tmp %>% gather(key,value,-date) %>% ggplot(aes(date,value,group=key))+geom_line()+facet_wrap(~key)
...
a <- best %>% 
  ...
  mutate(ret.CMA = map2_dbl(test, CMA,
         ...
         ret.HML = map2_dbl(test, HML,
                            ~ .x %>% 
                              ...
                              apply(2, function(x) exp(sum(x))) %>% 
                              mean()),
         ret.JPY_USD = map2_dbl(test, JPY_USD,
                            ~ .x %>% 
                              select(one_of(.y)) %>% 
                              apply(2, function(x) exp(sum(x))) %>% 
                              mean()),
  ...
  gather(key, value, -year, -last_month) %>% 
  mutate(last_month = formatC(last_month, width=2, flag="0")) %>% 
  mutate(date = ymd(as.numeric(str_c(year,last_month,"01"))))
